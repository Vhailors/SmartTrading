package com.smarttrading.app.ai.service;
...
public class StockPricePrediction {
    ...
    public void predict(PERIOD_CODE periodCode, String symbol, long candlesNum, boolean realValue) throws IOException, APIErrorResponse, APICommunicationException, APIReplyParseException, APICommandConstructionException {
        int batchSize = 64; // mini-batch size
        double splitRatio = 0.9; // 90% for training, 10% for testing
        int epochs = 1000; // training epochs

        log.info("Create dataSet iterator...");
        PriceCategory category = PriceCategory.OPEN; // CLOSE: predict close price
        StockDataSetIterator iterator = new StockDataSetIterator(batchSize, exampleLength, splitRatio, category, xtbDataSupplierService ,periodCode, symbol, candlesNum, realValue);
